...
>(({ className, ...props }, ref) => {
	...
	return (
		<Text
			...
			className={cn("pt-1 text-muted-foreground text-sm", className)}
			{...props}
		/>
	);
});
...
>(({ className, children, ...props }, ref) => {
	...
	return (
		<Animated.Text
			...
			className={cn("font-medium text-sm text-text-error", className)}
			{...props}
		>
			{body}
		</Animated.Text>
			...
			<View className="flex-row items-center gap-3">
				...
				{!!label && (
					<FormLabel
						...
						nativeID={formItemNativeID}
						onPress={() => {
							console.log("form.tsx: trigou onpress no FormLabel onPress");
						}}
